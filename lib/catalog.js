...
/** 一次移除多張圖片。positions 陣列順序不影響結果。 */
export function removeImagesAt(catalog, positions) {
  for (const position of positions) assertIndex(position, catalog.images.length);
  const drop = new Set(positions);
  const images = catalog.images.filter((_, i) => !drop.has(i));
  return { ...catalog, images };
}
